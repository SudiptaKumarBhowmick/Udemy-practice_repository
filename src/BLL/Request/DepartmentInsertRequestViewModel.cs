...
namespace BLL.Request
{
    ...
    public class DepartmentInsertRequestViewModelValidator : AbstractValidator<DepartmentInsertRequestViewModel> {
        ...
        public DepartmentInsertRequestViewModelValidator(IServiceProvider serviceProvider) {
            ...
            RuleFor(x => x.Name).NotNull().NotEmpty().MinimumLength(4).MaximumLength(25).MustAsync(NameExists).WithMessage("Name is already exists.");
            RuleFor(x => x.Code).NotNull().NotEmpty().MinimumLength(3).MaximumLength(10).MustAsync(CodeExists).WithMessage("Code is already exists.");
        }
